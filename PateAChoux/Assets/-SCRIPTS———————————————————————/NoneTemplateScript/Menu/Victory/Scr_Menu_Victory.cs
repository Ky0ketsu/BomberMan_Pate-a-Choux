...
public class Scr_Menu_Victory : MonoBehaviour
{
    ...
    [SerializeField] private GameObject winnerSlot;
    [SerializeField] private int currentWinner;
    [HideInInspector] private GameObject[] currentLoser;

    public void Victory()
    {
        slotPlayer = new GameObject[3];
        for (int i = 0; i < slotPlayer.Length; i++) slotPlayer[i] = GetComponentInChildren<VerticalLayoutGroup>().transform.GetChild(i).gameObject;
        currentLoser = new GameObject[3];
        winnerSlot.GetComponentInChildren<Image>().sprite = playerSprite[GameObject.Find("ActivePlayerParent").transform.GetChild(0).GetComponent<PlayerMove>().playerID];
